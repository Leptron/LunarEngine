...
#include <ultralight/AppCore/Defines.h>
#include <ultralight/Ultralight/platform/GPUDriver.h>
#include <vector>

namespace LunarUL {
    class GPUDriverImpl : public ultralight::GPUDriver {
    public:
        GPUDriverImpl();
        ...
        virtual void DrawGeometry(uint32_t geometry_id, uint32_t indices_count, uint32_t indices_offset, const ultralight::GPUState& state) = 0;
        virtual bool HasCommandsPending();
        virtual void DrawCommandList();
        virtual int batch_count() const;

        //inherited from GPU driver
